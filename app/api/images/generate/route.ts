...
export async function POST(request: NextRequest) {
  ...
  let productId = '';
  try {
    const { productId: pid, originalImageUrl, type, subType, customPrompt, category, sourceImageBuffer, quality, lane } = await request.json();
    productId = pid;
...
Output the enhanced image only.`;
    ...
    const { b64Json } = await editImage({ imageBuffer, prompt, size, quality, lane });
